...
tf.flags.DEFINE_string("checkpoint_dir", "./runs/1512188713/checkpoints", "Checkpoint directory from training run")
...
#    l2 = np.sqrt(sum(all_scores - y_test))
#    print("Total number of test examples: {}".format(len(y_test)))
    #print("Accuracy: {:g}".format(correct_predictions/float(len(y_test))))
#    print("L2 Loss: {:g}".format(l2)))


# Save the evaluation to a csv
with open('../datasets/anger-ratings-0to1.test.target.txt') as f:
    data = f.readlines()
data = [x.strip().split('\t') for x in data]
senid = np.array([x[0] for x in data])
mood = np.array([x[2] for x in data])

predictions_human_readable = np.column_stack((senid, np.array(x_raw), mood, all_scores))
#print predictions_human_readable
#out_path = os.path.join(FLAGS.checkpoint_dir, "..", "score.csv")
out_path = '../results/anger.txt'
